...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
                 ...
                 downsample=None, inplace=True, reduction=16):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, time_step, norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
                 bn_momentum=0.1, deep_stem=False, stem_width=32, inplace=True, reduction=16):
        self.inplanes = stem_width * 2 if deep_stem else 64
        super(ResNet, self).__init__()

        if deep_stem:
            self.conv1 = nn.Sequential(
                nn.Conv2d(time_step, stem_width, kernel_size=3, stride=2, padding=1,
                          bias=False),
            ...
            self.conv1 = nn.Conv2d(time_step, 64, kernel_size=7, stride=2, padding=3,
                                   bias=False)
    ...
    def forward(self, input):
        input = rearrange(input, 'b c t h w -> b (c t) h w')
        x1 = self.conv1(input)
        x1 = self.bn1(x1)
        x1 = self.relu(x1)
        x1 = self.maxpool(x1)
...
def resnet50(pretrained_model, time_step, use_SELayer, **kwargs):
    time_step = time_step * 3
    model = ResNet(Bottleneck, [3, 4, 6, 3], time_step, **kwargs)
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
    return model

def resnet101(pretrained_model, time_step, use_SELayer, **kwargs):
    time_step = time_step * 3
    model = ResNet(Bottleneck, [3, 4, 23, 3], time_step, **kwargs)
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
    return model


def resnet152(pretrained_model, time_step, use_SELayer, **kwargs):
    time_step = time_step * 3
    model = ResNet(Bottleneck, [3, 8, 36, 3], time_step, **kwargs)
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
    return model