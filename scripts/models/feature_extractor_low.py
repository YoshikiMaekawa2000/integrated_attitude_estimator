...
class BasicBlock_WO_SELayer(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, norm_layer=None,
                 bn_eps=1e-5, bn_momentum=0.1, downsample=None, inplace=True, reduction=16):
        super(BasicBlock_WO_SELayer, self).__init__()
...
def resnet18(pretrained_model, time_step, use_SELayer,**kwargs):
    time_step = time_step * 3
    model = ResNet(BasicBlock, [2, 2, 2, 2], time_step, **kwargs)
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
    return model

def resnet34(pretrained_model, time_step, use_SELayer, **kwargs):
    time_step = time_step * 3
    model = ResNet(BasicBlock, [3, 4, 6, 3], time_step, **kwargs)
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
    return model